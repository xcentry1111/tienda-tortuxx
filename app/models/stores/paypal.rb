  ...
  def process_card(card_data)
    ...
    options[:payer][:funding_instruments] = [{
      credit_card: {
        ...
        number: card_data[:number], #obtiene el numero de la tarjeta
        #Los datos que pide la tarjeta de credito y la gema gem "credit_card_validator"
        expire_month: card_data[:expire_month], 
        expire_year: card_data[:expire_year],
        cvv2: card_data[:cvv2],
        firt_name: card_data[:firt_name],
